...
purl("imputation_mice.Rmd", output = 'imputation_mice.R')
...
imputationPipeline<- function(
 ...
 # test_removed_rows = as.vector(read.csv(file.path(sub_path, 'test_removed_rows.csv')))$test_removed_rows
  ...
  print(duration_KNN)
  write.table(result_knn$train, file.path(sub_path, 'train_mice.csv.gz'), row.names=FALSE)
  write.table(result_knn$test, file.path(sub_path, 'test_mice.csv.gz'), row.names=FALSE)
  #---------------------------------------------------------------------------------------------------------------  
  # print("start dimv")
  # START = Sys.time()
