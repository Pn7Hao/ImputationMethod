...
purl("imputation_imputePCA.Rmd", output = 'imputation_imputePCA.R')
...
# path.X_train = file.path(sub_path, 'X_train_normed.csv.gz')
# r = read.table(path.X_train , sep=" ", header=T)
# 
# ncomp = estim_ncpPCA(as.matrix(r), ncp.min=2)
# 
# fit_train= imputePCA(r, ncp= 5)
...
imputationPipeline<- function(
 ...
 # test_removed_rows = as.vector(read.csv(file.path(sub_path, 'test_removed_rows.csv')))$test_removed_rows
  ...
  print(duration_KNN)
  write.table(result_knn$train, file.path(sub_path, 'train_imputePCA.csv.gz'), row.names=FALSE)
  write.table(result_knn$test, file.path(sub_path, 'test_imputePCA.csv.gz'), row.names=FALSE)
  #---------------------------------------------------------------------------------------------------------------  
  # print("start dimv")
  # START = Sys.time()
